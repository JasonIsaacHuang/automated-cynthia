...
def get_list_of_providers():
    url = base_url + '/home-loans/companies'
...
def get_list_of_all_products():
    url = base_url + '/home-loans/search?h_per_page=1000'
    response = requests.get(url)
    souped_response = BeautifulSoup(response.content, "html.parser")
    ...
    try:
        ...
        for idx, product in enumerate(products_urls):
            ...
            products_list.append(get_product_details(base_url + product))
    except Exception as e:
...
products = get_list_of_all_products()
